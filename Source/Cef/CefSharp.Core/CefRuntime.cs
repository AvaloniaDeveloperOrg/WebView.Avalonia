...
using CefSharp.Core.Platform.Windows;
using Dotnet.Toolkit.Helpers;
using Dotnet.Toolkit.Interfaces;
using Dotnet.Toolkit.Platform;
using Dotnet.Toolkit.Platform.Linux;
using Dotnet.Toolkit.Platform.Mac;
using Dotnet.Toolkit.Platform.Windows;

namespace CefSharp.Core;

public static unsafe class CefRuntime
{
    private const string _libc = "libc";

    private const string _windowlibcef = "libcef.dll";
    private const string _linuxlibcef = "";
    private const string _macoslibcef = "";
    ...
    public static void LoadLibrary()
    {
        ...
        string platformName = RuntimeExHelpers.Win;
        string architecture = "x86";
        string libcefName = _windowlibcef;
        uint dwFlags = 0;

        switch (_platform)
        {
            case CefRuntimePlatform.Windows:
                _libraryLoader = new Win32Loader();
                dwFlags = (uint)LoadLibraryFlags.LOAD_WITH_ALTERED_SEARCH_PATH;
                break;
            case CefRuntimePlatform.Linux:
                platformName = RuntimeExHelpers.Linux;
                libcefName = _linuxlibcef;
                _libraryLoader = new LinuxLoader();
                break;
            case CefRuntimePlatform.MacOS:
                platformName = RuntimeExHelpers.OsX;
                libcefName = _macoslibcef;
                _libraryLoader = new MacLoader();
                break;
            default:
                _libraryLoader = new NotSupportedLoader();
                break;
        }

        if (RuntimeExHelpers.IsX64())
            architecture = RuntimeExHelpers.X64;
        else if(RuntimeExHelpers.IsArm())
            architecture = RuntimeExHelpers.Arm;
        else if(RuntimeExHelpers.IsArm64())
            architecture = RuntimeExHelpers.Arm64;

        string libCefPath = Path.Combine(GetAssemblyLocationDirectory(), "runtimes", $"{platformName}-{architecture}", "native", libcefName);
        string libCefPath1 = Path.Combine(GetAssemblyLocationDirectory(), libcefName);
        var handle = _libraryLoader.LoadLibrary(libCefPath, IntPtr.Zero, dwFlags);
        if (handle == IntPtr.Zero)
            handle = _libraryLoader.LoadLibrary(libCefPath1, IntPtr.Zero, dwFlags);

        if (handle == IntPtr.Zero) throw new Exception($"LoadLibrary failed! Library is {libcefName}");
    ...
    {
        return Path.GetDirectoryName(typeof(CefRuntime).Assembly.Location);
    }
    ...
    private static void LoadLibraryWindows(string path)
    {
        NativeMethods.LoadLibraryEx(
            Path.Combine(path, _windowlibcef),
            IntPtr.Zero,
